...
func TestConfigDir(t *testing.T) {
	...
	t.Run("XDG_CONFIG_HOME is set", func(t *testing.T) {
		...
		expected := filepath.Join(tmpDir, "lemon3")
		if configDir != expected {
			if configDir == filepath.Join(homeDir(), ".lemon3") {
				t.Fatal("you should run this test without ~/.lemon3 dir")
			}
	...
	t.Run("XDG_CONFIG_HOME is not set", func(t *testing.T) {
		...
		expected := filepath.Join(homeDir(), ".lemon3")
		if configDir != expected {
			t.Fatalf("expected %v, got %v", expected, configDir)
		}
		if _, err := os.Stat(configDir); err != nil {
